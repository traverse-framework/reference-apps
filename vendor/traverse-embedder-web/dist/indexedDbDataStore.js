...
function dataStoreError(code, operation, reason) {
...
function validateRecord(record) {
    ...
    if (!Number.isSafeInteger(record.lamport_clock) ||
...
function mapDomFailure(operation, failure) {
    ...
    if (name === "SecurityError" ||
        ...
        name === "UnknownError") {
        return dataStoreError("persistence_unavailable", operation, "indexeddb_unavailable");
    }
...
async function digestForRecord(record, operation) {
    if (globalThis.crypto?.subtle === undefined) {
        throw dataStoreError("backend_failed", operation, "digest_unavailable");
    }
    let digest;
    try {
        digest = await globalThis.crypto.subtle.digest("SHA-256", new TextEncoder().encode(canonicalRecord(record)));
    }
    catch {
...
function isJsonValue(value) {
    return isJsonValueInner(value, new WeakSet());
}
function isJsonValueInner(value, ancestors) {
    if (value === null ||
        typeof value === "string" ||
    ...
    if (typeof value === "object" &&
        ...
        ancestors.add(value);
        const valid = Object.values(value).every((child) => isJsonValueInner(child, ancestors));
        ancestors.delete(value);
        return valid;
    }
    return false;
}
function parseEnvelope(value, requestedKey) {
    if (typeof value !== "object" || value === null) {
        throw dataStoreError("integrity_check_failed", "read", "malformed_envelope");
    }
    const envelope = value;
    if (envelope.format !== DATA_STORE_FORMAT) {
        throw dataStoreError("integrity_check_failed", "read", envelope.format === undefined ? "legacy_unverified" : "unknown_format_version");
    }
    if (envelope.classification !== "public") {
        throw dataStoreError("key_provider_required", "read", "private_record");
    }
    if ("record_key" in envelope ||
        "key_id" in envelope ||
        "nonce" in envelope ||
...
export class IndexedDbDataStore {
    ...
    lockRequest;
    closed = false;
    constructor(databaseName, classification, database, releaseLock, lockRequest) {
    ...
    static async open(config) {
        ...
        const factory = config.indexedDB === undefined ? defaultIndexedDb() : config.indexedDB;
        if (factory === undefined || factory === null) {
            throw dataStoreError("persistence_unavailable", "open", "indexeddb_unavailable");
        }
        ...
        try {
            lockRequest = locks
                .request(`${LOCK_NAME_PREFIX}${config.databaseName}`, { mode: "exclusive", ifAvailable: true }, async (lock) => {
                if (lock === null) {
                    signalRejected?.(dataStoreError("store_locked", "open", "exclusive_owner_active"));
                    return;
                }
                signalAcquired?.();
                await held;
                ...
                .catch(() => {
                signalRejected?.(dataStoreError("locking_unsupported", "open", "web_locks_failed"));
            });
        }
        catch {
        ...
        try {
            ...
            return new IndexedDbDataStore(config.databaseName, config.classification, database, releaseLock, lockRequest);
        }
        catch (error) {
            releaseLock();
            await lockRequest;
    ...
    request(operation, mode, createRequest) {
        return new Promise((resolve, reject) => {
            let transaction;
            let request;
            try {
