//! Embedded Traverse runtime host for meeting-notes shells.
//!
...
pub const DEFAULT_WORKFLOW_ID: &str = "meeting-notes.process";
pub const RUNTIME_MODE_EMBEDDED: &str = "Embedded";
pub const MANIFEST_ENV: &str = "MEETING_NOTES_MANIFEST";
...
fn collect_submit<E: TraverseEmbedderApi>(
    ...
    for event in &raw_events {
        ...
        if parsed.event_type == "capability_result" {
            let output = parsed.output.unwrap_or_else(MeetingNotesOutput::empty);
...
impl EmbeddedRuntime {
    ...
    pub fn init_default() -> Result<Self, HostError> {
        let path = resolve_manifest_path(None).ok_or_else(|| {
            HostError::Init(format!(
                "could not find manifests/meeting-notes/app.manifest.json (set {MANIFEST_ENV})"
            ))
        })?;
        Self::init(path)
    }
    ...
    pub fn submit_transcript(&mut self, transcript: &str) -> Result<HostRunResult, HostError> {
        collect_submit(
            &mut self.embedder,
            &self.events,
            &self.workflow_id,
...
impl TestEmbeddedRuntime {
    #[must_use]
    pub fn new(output: MeetingNotesOutput) -> Self {
