...
import { parseMeetingNotesOutput, type MeetingNotesOutput } from '../client/traverseOutput'

export const RUNTIME_MODE_EMBEDDED = 'Embedded'
export const DEFAULT_WORKFLOW_ID = 'meeting-notes.process'
export const DEFAULT_WORKSPACE = 'local-default'
export const DEFAULT_APP_ID = 'meeting-notes'
export const DEFAULT_MANIFEST_PATH = '/bundles/meeting-notes/app.manifest.json'
...
export interface HostRunResult {
  sessionId: string
  output: MeetingNotesOutput | null
  rawOutput: unknown
  events: TraceEvent[]
  error: string | null
...
export function createTestEmbedder(output: MeetingNotesOutput): TraverseEmbedderApi {
...
export async function initProductionEmbedder(
  manifestPath = import.meta.env.VITE_MEETING_NOTES_MANIFEST ?? DEFAULT_MANIFEST_PATH,
...
/** Submit `{ transcript }` to `meeting-notes.process` and collect terminal output. */
  ...
  for (const event of collected) {
    ...
    if (event.event_type === 'capability_result') {
      ...
      return withPresentation(
        {
          sessionId,
          output: parseMeetingNotesOutput(rawOutput),
          rawOutput,
          events,
          error: null,
        },
