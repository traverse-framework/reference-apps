...
const CAPABILITY_ID = import.meta.env.VITE_TRAVERSE_CAPABILITY_ID ?? 'meeting-notes.process'
...
function App() {
  ...
  const canSubmit = transcript.trim().length > 0 && runtimeStatus === 'online' && !isRunning

  const handleSubmit = useCallback((e?: React.FormEvent) => {
    e?.preventDefault()
    if (!canSubmit) return
    run(CAPABILITY_ID, { transcript })
  }, [canSubmit, run, transcript])
