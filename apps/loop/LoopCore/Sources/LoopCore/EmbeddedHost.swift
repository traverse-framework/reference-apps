    ...
    func submitTranscript(_ transcript: String) throws -> HostRunResult {
        ...
        for evt in runtimeEvents {
            ...
            if eventType == "capability_result", let out = evt.output {
                output = (try? JSONDecoder().decode(LoopOutput.self, from: out)) ?? .empty
                break
            }
        }
...
private func parseOutput(_ raw: Any?) -> LoopOutput {
    ...
    if let dict = value as? [String: Any],
       let parsed = LoopOutputParser.parse(dict) {
        return parsed
    }
    if let data = try? JSONSerialization.data(withJSONObject: value),
       let decoded = try? JSONDecoder().decode(LoopOutput.self, from: data) {
        return decoded
    }
    return .empty
}
