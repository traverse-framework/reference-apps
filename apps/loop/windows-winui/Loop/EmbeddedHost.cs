...
/// <see cref="InMemoryTraverseEmbedder"/> with scripted runtime-owned output.
/// </summary>
...
public static class EmbeddedHost
{
    ...
    public const string DefaultRelativeBundlePath = "Assets/bundles/loop";
    ...
    private sealed class ProductionEmbeddedHost : IEmbeddedHost
    {
        ...
        private HostRunResult DrainEvents(string sessionId)
        {
            ...
            return new HostRunResult(sessionId, output ?? LoopOutput.Empty, events, null)
                .WithPresentation(Array.Empty<EmbedderEventLike>());
        }
    ...
    private sealed class TestEmbeddedHost : IEmbeddedHost
    {
        ...
        public HostRunResult SubmitTranscript(string transcript)
        {
            ...
            foreach (var evt in runtimeEvents)
            {
                ...
                {
                    output = JsonSerializer.Deserialize<LoopOutput>(evt.Output, JsonOptions)
                        ?? LoopOutput.Empty;
                    break;
                }
            }
    ...
    private static LoopOutput ParseOutput(JsonElement? data)
    {
        if (data is null)
        {
            return LoopOutput.Empty;
        }
        ...
        return JsonSerializer.Deserialize<LoopOutput>(element.GetRawText(), JsonOptions)
            ?? LoopOutput.Empty;
    }
