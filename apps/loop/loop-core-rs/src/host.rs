//! Embedded Traverse runtime host for loop shells.
//!
...
pub const DEFAULT_WORKFLOW_ID: &str = "loop.wf1";
pub const RUNTIME_MODE_EMBEDDED: &str = "Embedded";
pub const MANIFEST_ENV: &str = "LOOP_MANIFEST";
...
fn collect_submit<E: TraverseEmbedderApi>(
    ...
    for event in &raw_events {
        ...
        if parsed.event_type == "capability_result" {
            let output = parsed.output.unwrap_or_else(LoopOutput::empty);
    ...
    pub fn init_default() -> Result<Self, HostError> {
        let path = resolve_manifest_path(None).ok_or_else(|| {
            HostError::Init(format!(
                "could not find manifests/loop/app.manifest.json (set {MANIFEST_ENV})"
            ))
        })?;
        Self::init(path)
    }
...
impl TestEmbeddedRuntime {
    #[must_use]
    pub fn new(output: LoopOutput) -> Self {
