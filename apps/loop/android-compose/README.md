...
Native Android client for the `loop` reference app.
...
Assets live under `app/src/main/assets/bundles/loop/` (including `runtime/runtime.wasm` + `runtime-release.json` after sync). The app copies them into `filesDir` on launch.
...
```bash
export TRAVERSE_REPO=/path/to/Traverse
cd apps/loop/android-compose
./gradlew test
./gradlew :app:assembleDebug
```

Unit tests inject `InMemoryTraverseEmbedder` via `InMemoryLoopHost`; fixtures use only the runtime-owned `LoopOutput` fields.
