...
/**
 * Embedded Traverse host for loop.
 *
 * Production uses the public Kotlin `dev.traverse.embedder` package.
 * Unit tests inject [InMemoryLoopHost] with scripted runtime-owned output.
 */
interface LoopHost {
...
class InMemoryLoopHost(
    ...
    companion object {
        ...
        fun parseOutput(raw: String): LoopOutput? = try {
            json.decodeFromString(LoopOutput.serializer(), raw)
        } catch (_: Exception) {
            null
        }

        fun withScriptedOutput(output: LoopOutput): InMemoryLoopHost =
            InMemoryLoopHost(json.encodeToString(LoopOutput.serializer(), output))
    }
}
 ...
 */
class ProductionLoopHost private constructor(
    ...
    override fun submitTranscript(transcript: String): HostRunResult = try {
        ...
        val output = runtimeEvents
            ...
            ?.output
            ?.let { InMemoryLoopHost.parseOutput(it) }
        HostRunResult(
            sessionId = result.sessionId,
            output = output,
            events = events,
            error = if (output == null) {
                "runtime returned no loop output"
            } else {
                null
            },
        ).withPresentation(emptyList())
    ...
    companion object {
        fun createOrNull(bundleRoot: File): ProductionLoopHost? {
