...
use crate::client::{TraverseClient, TraverseClientError};
...
pub async fn subscribe_events(
    ...
    workspace_id: &str,
    app_id: &str,
) -> Result<BoxStream<'static, Result<StateEvent, TraverseClientError>>, TraverseClientError> {
    let url = TraverseClient::app_events_url(base_url, workspace_id, app_id);
    let response = http
        .get(&url)
        ...
        |mut parser| async move {
            loop {
                ...
                match parser.byte_stream.next().await {
                    ...
                    Some(Err(err)) => {
                        return Some((
                            Err(TraverseClientError::Request(err.to_string())),
                            parser,
                        ));
                    }
                    None => return None,
