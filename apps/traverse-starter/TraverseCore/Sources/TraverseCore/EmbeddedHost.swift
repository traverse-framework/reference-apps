...
public protocol EmbeddedHostProtocol: AnyObject, Sendable {
    ...
    func submitNote(_ note: String) throws -> HostRunResult
}
...
public enum EmbeddedHost {
    ...
    public static let defaultRelativeBundlePath = "bundles/traverse-starter"
...
private final class ProductionEmbeddedHost: EmbeddedHostProtocol, @unchecked Sendable {
    ...
    func submitNote(_ note: String) throws -> HostRunResult {
        let input = try JSONSerialization.data(withJSONObject: ["note": note])
    ...
    func submitNote(_ note: String) throws -> HostRunResult {
        ...
        for evt in runtimeEvents {
            ...
            if eventType == "capability_result", let out = evt.output {
                output = (try? JSONDecoder().decode(TraverseStarterOutput.self, from: out)) ?? .empty
                break
            }
        }
...
private func parseOutput(_ raw: Any?) -> TraverseStarterOutput {
    ...
    if let dict = value as? [String: Any],
       let parsed = TraverseOutputParser.parse(dict) {
        return parsed
    }
    if let data = try? JSONSerialization.data(withJSONObject: value),
       let decoded = try? JSONDecoder().decode(TraverseStarterOutput.self, from: data) {
        return decoded
    }
    return .empty
}
