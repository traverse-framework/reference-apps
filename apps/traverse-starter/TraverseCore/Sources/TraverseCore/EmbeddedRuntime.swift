...
/// Result of a single workflow submission through the embedded runtime.
public struct HostRunResult: Sendable {
    ...
    public init(sessionID: String, output: TraverseStarterOutput?, events: [TraceEvent], error: String?) {
...
public protocol EmbeddedRuntimeHostProtocol: Sendable {
    ...
    func submit(note: String) throws -> HostRunResult
    func shutdown()
}

public enum EmbeddedRuntime {
    public static let runtimeMode = "Embedded"
    public static let defaultWorkflowID = "traverse-starter.pipeline"
    public static let defaultWorkspace = "local-default"
    public static let defaultAppID = "traverse-starter"
    ...
    public static let bundleSubpath = "bundles/traverse-starter"
    ...
    private final class ProductionEmbeddedHost: EmbeddedRuntimeHostProtocol, @unchecked Sendable {
        ...
        func submit(note: String) throws -> HostRunResult {
            let inputJSON = try JSONSerialization.data(withJSONObject: ["note": note])
        ...
        private func drainEvents(sessionID: String) -> HostRunResult {
            ...
            var output: TraverseStarterOutput?
            var error: String?
            ...
            for event in runtimeEvents {
                ...
                if type == "capability_result", let outputData = event.output {
                    output = TraverseOutputParser.parse(
            ...
            return HostRunResult(
                sessionID: sessionID,
                output: output ?? TraverseStarterOutput.empty,
                events: events,
